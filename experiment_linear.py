...
def experiment(
    ...
    # load dataset
    dataset_path = f'decision_transformer/envs/gym_ca/DATA/datasets/{dataset}.pkl'
    ...
    def get_batch(batch_size=256, max_len=K):
        ...
        def get_linear(ep, si, max_i):
            ...
            rad = ep['radii']
            js = []
            for i in range(si, max_i):
                js.append(
                    JointState(state[0][i][0], state[0][i][1], vel[0][i][0], rad[0], goal[0], goal[1], state[0][i][2], 
                              state[1][i][0], state[1][i][1], vel[1][i][0], rad[1],
                            #   state[2][i][0], state[2][i][1], vel[2][i][0], rad[2],
                            #   state[3][i][0], state[3][i][1], vel[3][i][0], rad[3]
                              )
                )
            return np.array(js)
        ...
        for i in range(batch_size):
            ...
            traj_obs = get_linear(episode, si, max)

            # get sequences from dataset
            ### CHANGED Reshape parameters for state
            s.append(traj_obs.reshape(1, -1, state_dim))    # Linear Layer
